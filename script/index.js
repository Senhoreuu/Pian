...
checkbox.addEventListener('change', ({ target }) => {
    if (target.checked) {
        ...
        localStorage.setItem('disabled-keys-piano', 'false');
        return;
    }
    ...
    localStorage.setItem('disabled-keys-piano', 'true');
});
...
document.addEventListener('keydown', (event) => {
    if (!keyDownMapper[event.key]) return;

    keyDownMapper[event.key]()
});

document.addEventListener('keyup', (event) => {
    if (!keyUpMapper[event.key]) return;
